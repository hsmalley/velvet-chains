...
# Captain Velvet orchestrates the temporal choreography with corsair precision1. **📜 READ THE SACRED DOCUMENTS:**
...
def generate_theatrical_commits(count: int) -> List[CommitPlan]:   - `BRIG_BRIEFING.mdx` - The main theatrical documentation
    ...
    return forge_commits_with_romance(count)   - This `QUARTERMASTER_CODEX.md` - Contributing guidelines
...
pub fn forge_commit_message(template: &str) -> String {

    // The warp drive purrs with mechanical precision3. **🏴‍☠️ EMBRACE THE THEME:**

    let story = select_corsair_narrative();   - Use **spectacular corsair language** in all communications

    format!("⚔️ {}: {}", template, story)   - Maintain **BDSM space-pirate romance** aesthetic consistently
...
🏴‍☠️ The cosmic brig now features enhanced capabilities:   cd velvet-chains

- ⚔️ Added theatrical element X with corsair precision   git checkout -b feature/your-spectacular-enhancement

- 🌌 Enhanced quantum entanglement protocols    ```

- 💎 Optimized for maximum theatrical spectacle

2. **🔧 PREPARE YOUR DEVELOPMENT BRIG**

Captain Velvet approved this enhancement during morning rituals.   ```bash
...
- Use **theatrical names**: `voidlight_choreographer.py`, `BRIG_BRIEFING.mdx`   cargo install --path voidlight_engine --force

- Include **corsair terminology** in functions and variables   ```

- **Spectacular documentation** with emoji and theatrical language

- **CONSENT_DECREE.txt**, **CAPTAINS_LOG.md**, **QUARTERMASTER_CODEX.md**3. **🎭 CREATE THEATRICAL EXCELLENCE**

   - **Maintain theme consistency** - Every addition must enhance the corsair aesthetic

### **DOCUMENTATION STANDARDS** ✨   - **Add comprehensive tests** - Even theatrical tools need reliability

   - **Document with flair** - Update relevant `.md` files with spectacular language

**Always include:**   - **Follow code quality** - All pre-commit hooks must pass with flying colors

- 🛡️ **Safe word respect** in appropriate contexts

- ⚔️ **Theatrical language** with corsair flair4. **✨ SUBMIT YOUR THEATRICAL MASTERPIECE**

- 🏴‍☠️ **Emoji enhancement** for visual spectacle   ```bash

- 🎭 **Character references** (Captain Velvet, etc.)   # Ensure quality standards are met
...
logger.info("✨ Captain Velvet's choreography engine purrs with satisfaction")   - 🎭 Enhanced theatrical element X with spectacular flair

logger.warning("🏴‍☠️ Quartermaster Siren reports minor turbulence in the void")   - ⚔️ Maintained BDSM space-pirate consistency throughout
...
- 🌟 **Use corsair language** while remaining helpful and clear### **🌟 HIGH-PRIORITY THEATRICAL ENHANCEMENTS:**

- 🌟 **Explain technical concepts** with theatrical metaphors

- 🌟 **Honor consent boundaries** - never compromise safety for spectacle- 🎨 **Visual Spectacle Improvements** - Enhance SVG generation, add new heatmap styles

- 🌟 **Enhance the narrative** while providing practical assistance- 📚 **Documentation Theater** - Expand the corsair narrative in existing docs

- ⚔️ **Rust Engine Enhancements** - Add new commit message story templates

**EXAMPLE INTERACTIONS:**- 🐍 **Python Choreographer Features** - New distribution modes, date handling

```- 🎭 **Quality Assurance Theater** - Improve pre-commit hooks with more flair

User: "How do I run the Python script?"

### **🏴‍☠️ THEATRICAL GUIDELINES:**

Response: "⚔️ Magnificent corsair! To unleash the SPECTACULAR choreography engine:

**Code Comments:**

```bash```rust

python3.12 voidlight_choreographer.py -n 50 --preview-only --svg-out corsair_spectacle.svg// Captain Velvet adjusts her corseted armor before engaging the warp drive

```if let Some(commit_hash) = git_engine.forge_theatrical_commit() {

    // The cosmic brig trembles with anticipation

This conjures 50 commits from the cosmic void while maintaining PURE PREVIEW SAFETY! 🏴‍☠️✨"    logger.spectacular_log("⚔️ Commit forged with corsair precision!");
...
- ⚔️ **Rust engine** compilation without flaws

- 💎 **Clippy dominatrix** approval---
...
def test_corsair_functionality():**🌟 ALL CONTRIBUTIONS MUST:**
    ...
    """Ensures our theatrical spectacle maintains reliability! ⚔️"""1. **Pass pre-commit quality assurance** - No exceptions for theatrical excellence

    # Captain Velvet demands nothing less than perfection2. **Maintain theme consistency** - Every line must enhance the corsair aesthetic

    result = choreograph_spectacular_commits(count=5)3. **Include comprehensive tests** - Reliability within the theatrical framework

    assert len(result) == 5, "🏴‍☠️ The cosmic brig demands precise counts!"4. **Honor consent boundaries** - Never compromise safety for spectacle
...
## 🛡️ **CONSENT & SAFETY FRAMEWORK** - **Technical Excellence:** Does the code meet engineering standards?

- **Consent Framework:** Are all safety boundaries properly respected?

### **ALWAYS REMEMBER** 🌟- **Theme Enhancement:** Does this addition improve the overall aesthetic?

- **Documentation Quality:** Is the theatrical narrative properly enhanced?

- **Fiction only**: Never suggest using tools to deceive or impersonate real work

- **Safe word sacred**: Honor "fiction" as immediate cessation signal**💫 REVIEWER RESPONSIBILITIES:**
...
### **WHEN USERS ASK ABOUT REAL-WORLD USAGE** ⚠️
...
- 🎭 **Pure theatrical magic**- **🔄 Pull Request Reviews** - Code review discussions with corsair flair

- **📚 GitHub Discussions** - Community conversations and corsair storytelling

**Remember the corsair's motto:**- **⚔️ Project Board** - Tracking theatrical development with spectacular organization

> _"In preview we trust, in consent we flourish, in theater we transcend"_ 🏴‍☠️✨

### **🎭 COMMUNICATION GUIDELINES:**

**May your assistance sparkle with corsair magic while honoring all boundaries!**
...
*Safe word honored • Boundaries respected • Code perfected • Theater transcendent* 🛡️- 🌟 Provide constructive feedback with encouraging spectacle
