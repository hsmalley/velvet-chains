...
const LORE_DIR = typeof window === 'undefined' && path ? path.join(process.cwd(), '_LORE_') : '_LORE_';
...
function loadLoreFiles(): LoreFileMeta[] {
  ...
  if (!fs || !path) { CACHE = []; return CACHE; }
  if (!fs.existsSync(LORE_DIR)) { CACHE = []; return CACHE; }
  ...
  for (const ent of entries) {
    ...
    lore.push({ index, filename: ent.name, title: metaTitle, content: body, html, tags, draft, order });
  }
  CACHE = lore.sort((a, b) => {
...
const tocStyle: React.CSSProperties = { listStyle: 'none', padding: 0, margin: '0.5rem 0 1.5rem', display: 'grid', gap: '0.25rem', fontSize: '0.9rem' };
const detailsStyle: React.CSSProperties = { border: '1px solid #9333ea', borderRadius: '10px', padding: '0.75rem 1rem', marginBottom: '1rem', background: 'rgba(0,0,0,0.45)' };
const summaryStyle: React.CSSProperties = { cursor: 'pointer', fontWeight: 600, color: '#fbbf24', display: 'flex', gap: '0.5rem', alignItems: 'center' };
const metaStyle: React.CSSProperties = { fontSize: '0.65rem', color: '#c084fc', margin: '0.4rem 0 0.8rem' };
...
function buildStaticHTML(): string {
  ...
  const toc = `<nav aria-label="Lore index"><strong style="font-size:0.85rem;letter-spacing:.5px">Lore Index</strong><ul style="${inlineStyle(tocStyle)}">` +
    slugs.map(item => `<li><a href="#${item.slug}" style="text-decoration:none;color:#d946ef">${String(item.index).padStart(2,'0')} · ${escapeHtml(item.title)}</a></li>`).join('') + '</ul></nav>';
  const body = slugs.map(item => {
    const tags = item.tags && item.tags.length ? ` <span style=\"margin-left:.5rem\">• ${item.tags.map(escapeHtml).join(', ')}<\/span>` : '';
  ...
  const footer = '<footer style="margin-top:1rem;font-size:0.6rem;text-align:right;opacity:.75">⚔️ Static lore conjured at build export • Safe word: <code>fiction</code></footer>';
...
function inlineStyle(obj: Record<string, any>): string { return Object.entries(obj).map(([k,v]) => `${k.replace(/[A-Z]/g,m=>'-'+m.toLowerCase())}:${String(v)}`).join(';'); }
