...
Promote the included script into a full-fledged subcommand:

```bash
chmod +x git-snark
ln -s "$(pwd)/git-snark" /usr/local/bin/git-snark
```
...
It stages (if asked), commits, and auto-appends a random flourish—all consensually outrageous.
