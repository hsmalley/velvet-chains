...
Already installed the binary? One-liner: `git snark install-hook` (add `--force` to overwrite).
...
```bash
git snark --commit -a -m "Refactor release valve" -- -- path/to/file
```
...
Every flag mirrors `generate_snark.py`, now globally available from any repo.
